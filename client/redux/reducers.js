  ...
  switch (action.type) {
    ...
    case types.WITHDRAW_BALANCE:
      ...
      case types.UPDATE_PROJECT_STATE:
        const { projectId: updatedProjectId, newState } = action.payload; // Rename projectId to avoid redeclaration
        var updatedState = state.projects.map((data) => {
          if (data.address === updatedProjectId) { // Use updatedProjectId here
            data["state"] = newState;
          }
          return data;
        });
